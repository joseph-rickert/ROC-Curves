...
This post makes the case for using arclength as a complementary measure to AUC.
...
### Select the Data Set and Split the Data
```{r}
#| message: FALSE
#| warning: FALSE
#| code-fold: TRUE
...
### Parital AUC and Arc Length (FPR ≤ 0.5)
...
#| code-summary: "Show Code"
legend_labels <- setNames(
  paste0(metrics$model_norm, "\npAUC (≤0.5) = ", sprintf("%.3f", metrics$pAUC),
         "\nArc (≤0.5) = ", sprintf("%.3f", metrics$ArcHalf)),
  metrics$model_norm
)
...
ggplot(smooth_results, aes(x = FPR, y = TPR, color = model_norm)) +
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey50") +
  coord_fixed(ratio = 1, xlim = c(0,1), ylim = c(0,1), expand = FALSE) +
  labs(title = "Smoothed Partial ROC Curves (FPR ≤ 0.5)",
       x = "False positive rate (FPR)",
       y = "True positive rate (TPR)",
       color = NULL) +
  scale_color_manual(values = color_values, labels = legend_labels) +
...
🧠 Why this matters
These threshold‑based plots highlight decision regions: where small changes in threshold cause large changes in sensitivity or specificity.
...
Great observation — the “truncation at 0.5” you’re seeing for the decision tree curves is not a plotting bug, it’s a property of how the tree produces predictions.

---

### 🔎 Why the cutoff happens at 0.5
...
- That means the ROC curve is “chunky” and the threshold plots are truncated at the minimum and maximum leaf probabilities.  
- In your case, the tree’s leaves only output probabilities ≥ 0.5 for the positive class, so the threshold plots stop at 0.5.

---

### 🧠 Clinical/analytical significance
- This is not an error in your smoothing code — it reflects the **limited resolution of the decision tree’s probability estimates**.  
- It explains why decision trees often have poor calibration: they can’t finely adjust thresholds because they only produce a handful of distinct probability values.  
- That’s why ensemble methods (random forests, gradient boosting) are preferred when you want a full ROC curve across the entire [0,1] threshold range.

---

👉 If you’d like, I can show you how to **inspect the leaf probabilities of your decision tree** so you can see exactly which discrete values are being produced and why the ROC curve truncates at 0.5. Would you like me to walk through that?
