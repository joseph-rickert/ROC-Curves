...
It is a well known result that the slope of the tangent to the ROC curve at any point is equal to the the instantaneous likelihood ratio at that point. [Choi (1998)](https://academic.oup.com/aje/article-abstract/148/11/1127/123231?redirectedFrom=PDF). It is easy to compute the derivatives for the smoothed curves. In the plot below we see how slope varies for each of our three models.
...
# # --- Compute ROC geometry including derivative (likelihood ratio) ---
# compute_roc_geometry <- function(df, n_grid = 1001) {
#   f <- make_spline(df)
#   xg  <- seq(0, 1, length.out = n_grid)
#   yg  <- f(xg, deriv = 0)
#   y1g <- f(xg, deriv = 1)   # slope = likelihood ratio
#   y2g <- f(xg, deriv = 2)
#   y3g <- f(xg, deriv = 3)
# 
...
# 
#   tibble(FPR = xg, TPR = yg,
#          dTPR_dFPR = y1g,   # likelihood ratio
#          d2TPR_dFPR2 = y2g,
#          d3TPR_dFPR3 = y3g,
#          kappa = kappa,
#          dkappa_dFPR = dkappa)
...
# --- Compute ROC geometry: keep all derivatives + curvature ---
compute_roc_geometry <- function(df, n_grid = 1001) {
  f <- make_spline(df)
  xg  <- seq(0, 1, length.out = n_grid)
  yg  <- f(xg, deriv = 0)
  y1g <- f(xg, deriv = 1)   # slope = likelihood ratio (Choi 1998)
  y2g <- f(xg, deriv = 2)
  y3g <- f(xg, deriv = 3)
...
plot_dcurvature <- function(geom_df, model_label, color_hex) {
  ggplot(geom_df, aes(x = FPR, y = dkappa_dFPR)) +
    geom_hline(yintercept = 0, color = "grey80") +
    geom_line(color = color_hex, linewidth = 0.9) +
...
# --- Main: build 3x2 grid (curvature | dκ/dFPR) ---
plot_roc_geometry_grid <- function(smooth_results, n_grid = 1001) {
  model_order <- c("Decision Tree", "Logistic Regression", "SVM")
  models <- intersect(model_order, unique(smooth_results$model_norm))
  stopifnot(length(models) > 0)
...
# --- Usage ---
# Build the unified derivative data frame
derivative_df <- make_derivative_df(smooth_results, n_grid = 2001)

# Plot curvature and its derivative
grid_plot <- plot_roc_geometry_grid(smooth_results, n_grid = 2001)
print(grid_plot)

# Inspect the unified data frame
head(derivative_df)
